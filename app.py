...
from data import worktime_104, worktime_1111, worktime_518, salary_dict, salary_type_104, salary_type_1111, salary_type_518
...
@app.route('/')
def index():
    return render_template('index.html', search_history=search_history, area_dict=area_dict, category_dict=jobcat_104 ,work_time_dict=worktime_104, salary_dict=salary_dict)

@app.route('/search')
def search():
    global job_list, keyword, keyword_trans, area_list, area_num_104, area_num_1111, area_num_518, category_list, work_time,salary
    job_list.clear()
    area_list.clear()
    area_num_104.clear()
    area_num_1111.clear()
    ...
    search_url = 'http://127.0.0.1:5000/search?keyword={}&area={}&category={}&work-time={}&salary={}'.format(keyword_trans, '&area'.join(area_list), '&category'.join(category_list), work_time, salary)
    ...
    #     url = 'https://www.104.com.tw/jobs/search/?ro=0&kwop=7&keyword={}&area={}&cat={}&ro={}&sctp={}&scmin={}&page={}&jobsource=2018indexpoc'.format(keyword_trans, ','.join(area_num_104), ','.join(cat_num_104), work_time_104, salary_type_num, salary_num, page_num)
    ...
    # count = 1
    # while count <= 5:
    #     response_1111 = requests.get('https://www.1111.com.tw/job-bank/job-index.asp?ks={}&d0={}&c0={}&ts={}&st={}&sa0={}&page={}&si=1'.format(keyword_trans, ','.join(area_num_1111), ','.join(cat_num_1111), work_time_1111, salary_type_num, salary_num, count))
    ...
    for page_num in range(1, 6):
        url = 'https://www.518.com.tw/job-index-P-{}.html?ad={}&aa={}%2C&ab={}%2C&ai={}&ak={}&ak_min={}'.format(page_num, keyword_trans, ','.join(area_num_518), ','.join(cat_num_518), work_time_518, salary_type_num, salary_num)
